...
export function createAIPlugin(): KernelPlugin {
  ...
  nodes.set('Wo', [
    ...
    createNode('ai_build_attention_mask', (g) => {
  ...
  nodes.set('Sek', [
    createNode('ai_compute_qk_attention', (g) => {
      // Compute Q @ K^T
      const result = {
        ...g,
        name: 'QKAttentionGraph',
        nodes: [...g.nodes],
        edges: [...g.edges, ['q_tensor', 'k_tensor'] as [string, string]],
        data: g.data
      };
    ...
    createNode('ai_apply_attention_to_v', (g) => {
      // Apply attention to V
      const result = {
        ...g,
        name: 'AttentionOutputGraph',
        nodes: [...g.nodes, 'attention_output'],
        edges: [...g.edges, ['attention_probs', 'v_tensor'] as [string, string]],
        data: g.data
      };
    ...
    createNode('ai_output_projection', (g) => {
      // Output projection
      const result = {
        ...g,
  ...
  return createPlugin('AIPlugin', nodes);
}
 ...
 */
export function createLoRAPlugin(): KernelPlugin {
  const nodes = new Map<string, Array<{name: string, transform: any}>>();
  
  // Wo Fold: Build LoRA structures
  nodes.set('Wo', [
    createNode('lora_build_matrices', (g) => {
  ...
  // Sek Fold: Apply LoRA
  nodes.set('Sek', [
    createNode('lora_apply_adaptation', (g) => {
      // Apply LoRA: W + BA
      const result = {
        ...g,
        name: 'LoRAAdaptedGraph',
        nodes: [...g.nodes, 'adapted_weights'],
        edges: [...g.edges, ['lora_A', 'lora_B'] as [string, string]],
        data: g.data
      };
  ...
  return createPlugin('LoRAPlugin', nodes);
}
 ...
 */
export function createMoEPlugin(): KernelPlugin {
  const nodes = new Map<string, Array<{name: string, transform: any}>>();
  
  // Wo Fold: Build MoE structures
  nodes.set('Wo', [
    createNode('moe_build_router', (g) => {
  ...
  nodes.set('Sek', [
    createNode('moe_route_tokens', (g) => {
      // Route tokens to experts
      const result = {
        ...g,
        name: 'MoERoutedGraph',
        nodes: [...g.nodes, 'routed_tokens'],
        edges: [...g.edges, ['gate_network', 'expert_1'] as [string, string]],
        data: g.data
      };
    ...
    createNode('moe_combine_outputs', (g) => {
  ...
  return createPlugin('MoEPlugin', nodes);
}
