...
import { createAIPlugin, createLoRAPlugin, createMoEPlugin } from '../plugins/AIPlugin';
import { createPhysicsPlugin } from '../plugins/PhysicsPlugin';
...
async function main() {
  ...
  const baseKernel = new KuhulKernel({
    deterministic: true,
    replayEnabled: true,
    maxCycles: 1
  });
  ...
  const aiKernel = new KuhulKernel({
    deterministic: true,
    replayEnabled: true,
    maxCycles: 1
  });
  ...
  aiKernel.installPlugin(createAIPlugin());
  ...
  const multiKernel = new KuhulKernel({
    deterministic: true,
    replayEnabled: true,
    maxCycles: 1
  });
  ...
  multiKernel.installPlugin(createAIPlugin());
  multiKernel.installPlugin(createLoRAPlugin());
  multiKernel.installPlugin(createMoEPlugin());
  ...
  const physicsKernel = new KuhulKernel({
    deterministic: true,
    replayEnabled: true,
    maxCycles: 1
  });
  ...
  physicsKernel.installPlugin(createPhysicsPlugin());
